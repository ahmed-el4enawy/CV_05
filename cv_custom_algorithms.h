    ...
    int max_matches = 100)
{
    ...
    for (int y = 0; y < img1.rows; ++y)
        for (int x = 0; x < img1.cols; ++x) {
            if (img1.channels() == 3) {
                canvas.at<cv::Vec3b>(y, x) = img1.at<cv::Vec3b>(y, x);
            } else {
                uchar v = img1.at<uchar>(y, x);
    ...
    for (int y = 0; y < img2.rows; ++y)
        for (int x = 0; x < img2.cols; ++x) {
            if (img2.channels() == 3) {
                canvas.at<cv::Vec3b>(y, x + offset_x) = img2.at<cv::Vec3b>(y, x);
            } else {
                uchar v = img2.at<uchar>(y, x);
                canvas.at<cv::Vec3b>(y, x + offset_x) = cv::Vec3b(v, v, v);
            }
        }

    // Draw keypoints as small circles
    for (auto& kp : kps1) {
        draw_circle(canvas, {(int)kp.x, (int)kp.y}, 3,
                    cv::Scalar(0, 255, 0), 1);
    }
    for (auto& kp : kps2) {
        draw_circle(canvas, {(int)kp.x + offset_x, (int)kp.y}, 3,
                    cv::Scalar(0, 255, 0), 1);
    }
    ...
    for (int i = 0; i < count; ++i) {
        ...
        cv::Point p1((int)kps1[m.idx1].x, (int)kps1[m.idx1].y);
        cv::Point p2((int)kps2[m.idx2].x + offset_x, (int)kps2[m.idx2].y);
...
// ═══════════════════════════════════════════════════════════
//  EIGENFACES / PCA (Moved from cv_face_algorithms.h)
// ═══════════════════════════════════════════════════════════
